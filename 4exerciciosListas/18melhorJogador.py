...
while(numeroAtleta != 0):
    numeroAtleta = int(input("Número do jogador (0=fim): "))
...
for votosAtleta in votosAtletas:
    if (votosAtleta > 0):
        print("{0}\t{1}\t{2}".format(contador, votosAtleta, votosAtleta / float(totalVotos) * 100))
        if (votosAtleta > votosAtletas[melhorJogador]):
            melhorJogador = contador - 1
    contador += 1

print("O melhor jogador foi o numero {0}, com {1} votos, correspondendo a {2} do total de votos".format(melhorJogador + 1, votosAtletas[melhorJogador], votosAtletas[melhorJogador] / float(totalVotos) * 100))
